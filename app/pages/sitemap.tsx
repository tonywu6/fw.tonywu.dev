...
import { Link, useHref, useLoaderData } from "react-router";
...
export async function loader() {
  ...
  return {
    pages: await import("./urls")
      .then(({ urls }) => Object.entries(urls))
      .then((urls) => urls.map(([slug, url]) => ({ slug, url })))
      .then(
        (pages) => (
          pages.sort(({ url: u1 }, { url: u2 }) => u1.localeCompare(u2)),
          pages
        ),
      ),
  };
